...
def _unpickle(data):
    # Specify latin1 encoding to prevent raw byte values from causing an ASCII decode
    # error.
...
class RenPyArchive:
    ...
    # For backward compatibility, otherwise Python3-packed archives won't be read by
    # Python2
    PICKLE_PROTOCOL = 2
    ...
    def read(self, filename):
        ...
        if filename not in self.files and filename not in self.indexes:
            raise IOError(
                errno.ENOENT,
                f"the requested file {filename} does not exist in the given Ren'Py "
                "archive",
            )
        ...
        # Check our simplified internal indexes first, in case someone wants to read a
        # file they added before without saving, for some unholy reason.
        if filename in self.files:
            self.verbose_print(f"Reading file {filename} from internal storage...")
        ...
        # We need to read the file from our open archive.
            ...
            self.verbose_print(
                f"Reading file {filename} from data file {self.file}... (offset = "
                f"{offset}, length = {length} bytes)"
            )
            self.handle.seek(offset)
    ...
    def change(self, filename, contents):
        # Our 'change' is basically removing the file from our indexes first, and then
        # re-adding it.
    ...
    def add(self, filename, contents):
        ...
        if filename in self.files or filename in self.indexes:
            raise ValueError(f"file {filename} already exists in archive")

        self.verbose_print(
            f"Adding file {filename} to archive... (length = {len(contents)} bytes)"
        )
    ...
    def remove(self, filename):
        if filename in self.files:
            ...
            raise IOError(
                errno.ENOENT,
                f"the requested file {filename} does not exist in this archive",
            )
    ...
    def save(self, filename=None):
        ...
        archive.write(
            codecs.encode(pickle.dumps(indexes, self.PICKLE_PROTOCOL), "zlib")
        )
        # Now write the header.
        self.verbose_print(
            f"Writing header to archive file... (version = RPAv{self.version})"
        )
        archive.seek(0)
        if self.version == 3:
            archive.write(
                codecs.encode(f"{self.RPA3_MAGIC}{offset:016x} {self.key:08x}\n")
            )
        else:
            archive.write(codecs.encode(f"{self.RPA2_MAGIC}{offset:016x}\n"))
        # We're done, close it.
        archive.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="The FILE argument can optionally be in ARCHIVE=REAL format, mapping a "
        "file in the archive file system to a file on your real file system. An "
        "example of this: rpatool -x test.rpa script.rpyc=/home/foo/test.rpyc",
        add_help=False,
    )

    parser.add_argument(
        "archive", metavar="ARCHIVE", help="The Ren'py archive file to operate on."
    )
    parser.add_argument(
        "files",
        metavar="FILE",
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="List files in archive ARCHIVE."
    )
    parser.add_argument(
        "-x", "--extract", action="store_true", help="Extract FILEs from ARCHIVE."
    )
    parser.add_argument(
        "-c", "--create", action="store_true", help="Creative ARCHIVE from FILEs."
    )
    parser.add_argument(
        "-d", "--delete", action="store_true", help="Delete FILEs from ARCHIVE."
    )
    parser.add_argument(
        "-a", "--append", action="store_true", help="Append FILEs to ARCHIVE."
    )
    ...
    parser.add_argument(
        ...
        "--key",
        metavar="KEY",
        help="The obfuscation key used for creating RPAv3 archives, in hexadecimal "
        "(default: 0xDEADBEEF).",
    )
    parser.add_argument(
        "-p",
        "--padding",
        metavar="COUNT",
        help="The maximum number of bytes of padding to add between files "
        "(default: 0).",
    )
    parser.add_argument(
        "-o",
        "--outfile",
        help="An alternative output archive file when appending to or deleting from "
        "archives, or output directory when extracting.",
    )
    ...
    try:
        ...
        print(
            f"Could not open archive file {archive} for reading: {e}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if arguments.create or arguments.append:
        ...
        def add_file(filename):
            # If the archive path differs from the actual file path, as given in the
            # argument,
            ...
            if os.path.isdir(filename):
                for file in os.listdir(filename):
                    # We need to do this in order to maintain a possible ARCHIVE=REAL
                    # mapping between directories.
                ...
                try:
                    ...
                    print(
                        f"Could not add file {filename} to archive: {e}",
                        file=sys.stderr,
                    )
        ...
        try:
            ...
            print(f"Could not save archive file: {e}", file=sys.stderr)
        ...
        for filename in arguments.files:
            try:
                ...
                print(
                    f"Could not delete file {filename} from archive: {e}",
                    file=sys.stderr,
                )
        ...
        for filename in files:
            ...
            try:
                ...
                print(
                    f"Could not extract file {filename} from archive: {e}",
                    file=sys.stderr,
                )
    elif arguments.list:
